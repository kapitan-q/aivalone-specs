...
- [ ] Класс должен быть расположен в `src/Context/Shared/Domain/Model/AggregateRoot.php`
- [ ] Быть абстрактным классом
- [ ] Не знать о способе публикации событий (EventBus, Message Broker и т.д.)
...
- [ ] `recordEvent()` - защищённый метод для регистрации события внутри агрегата
- [ ] `pullEvents()` - возвращает ВСЕ накопленные события и ОЧИЩАЕТ внутреннее хранилище
- [ ] `hasEvents()` - проверяет наличие непубликованных событий
...
- [ ] Внутренний массив `$recordedEvents` для хранения событий
- [ ] События добавляются только внутри агрегата через `recordEvent()`
- [ ] События недоступны для прямой модификации извне

## Инварианты

- [ ] События накапливаются в порядке их возникновения
- [ ] После вызова `pullEvents()` внутреннее хранилище пусто
- [ ] `pullEvents()` может быть вызван многократно (последующие вызовы возвращают пустой массив)
